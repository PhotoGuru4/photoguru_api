...
async function main() {
  ...
  console.log('--- Seeding 15 Concepts ---');
  ...
  for (let i = 3; i <= 15; i++) {
    ...
    await prisma.message.create({
      data: { chatRoomId: room.id, senderId: c1.id, content: `Message #${i}` },
    });
  }
